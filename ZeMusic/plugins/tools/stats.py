...
from ZeMusic.utils.database import (get_global_tops,
...
async def gstats_global(client, message: Message, _):
    ...
    title = title.title()
    final = f"اكثر اغنية تم تشغيلها بميرا\n\n**العنوان :** {title}\n\nتم تشغيلها** {co}0 **مرة"
...
@app.on_message(filters.command("احصائيات ميرا العالمية", [".", ""]) & filters.group)
@languageCB
async def gstats_global(client, message: Message, _):
    mystic = await message.reply_text(_["gstats_1"])
    stats = await get_global_tops()
